      ...
      Tconv = (1.0/bkmpcgs) *( Gcgs**3 * Msyscgs**2 *            &
           PKcgs**(-xn))**(1.0/(3.0-xn))
      
      Sconv = (Gcgs**(2.0*xn) * Msyscgs**(1.0+xn) *              &
            PKcgs**(-2.0*xn))**(1.0/(3.0-xn))
      
      Pconv = (Gcgs**(3.0+3.0*xn) * Msyscgs**(2.0+2.0*xn) *      &
            PKcgs**(-4.0*xn))**(1.0/(3.0-xn))

      Dconv = (Gcgs**(-xn) * Msyscgs**(1.0-xn) *                 &
            PKcgs**(xn))**(1.0/(3.0-xn))
      ...
      subroutine Initengtable()
        ...
        do I = 1, TTABLE
          ...
          engtable(I) = bkmpcgs*0.5d0*temptable(I)*(1.5d0        &
           ...
           engtable(I) = engtable(I)*xabun + (bkmpcgs*yabun*     &
                       .25d0 +bkmpcgs*zabun/16.78d0)*            & 
                       temptable(I)*1.5d0  !from boss 1984 3.118d7 and 1.247d8
        ...
        do I = 1, TTABLE
         write(456, "(5(1X,1pe15.8))")temptable(I),1.+1./        &
                    (cvsm(I)*muc/bkmpcgs),                       &
           ...
           do K = 1, KMAX2
             do J = 1, JMAX2
               ...
               if (rho(J,K,L) >= limiter) then
                 ...
                 eflufftot = eflufftot + (eps(J,K,L)-dummy)*     &
                             rhf(J) * rof3n*dtheta*zof3n
                 eps(J,K,L) = dummy
...
!$OMP END DO
      ...
      subroutine TempFindSpec(eps,rho,temp)! like TempFind, but for a single cell. 
         ...
         if (eng < engtable(1)) then
            ...
            temp = temptable(I-1) + (temptable(I)-temptable(I-1))&
                   /(engtable(I)-engtable(I-1))*                 &
                    (eng - engtable(I-1))
