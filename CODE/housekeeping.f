        ...
        DO L=1,lmax,1
          DO K=2,kmax1,1
            ...
            if (ABS(u(J,K,L)).gt.VLIM) then
              ...
              s(J,K,L)=u(J,K,L)*0.5*(rho(J,K,L)+rho(J-1,K,L))
            endif
            if (ABS(w(J,K,L)).gt.VLIM) then
              w(J,K,L)=SIGN(VLIM,w(J,K,L))
              t(J,K,L)=w(J,K,L)*0.5*(rho(J,K,L)+rho(J,K-1,L))
            endif
...
!$OMP DO SCHEDULE(STATIC)
        DO l=1,lmax,1
          DO k=2,kmax1,1
            ...
            SPEED2=0.5*ABS(u(j+1,k,l)+u(j,k,l))
            SPEED3=0.5*ABS(w(j,k+1,l)+w(j,k,l))
            SPEED4=ABS(JN(j,k,l)/rhf(j))
            speed5=4.*sqrt(qrr(j,k,l))
            speed6=4.*sqrt(qzz(j,k,l))
            speed7=4.*sqrt(qtt(j,k,l))
...
C and possibility of rounding errors yielding  DELT(new) > DELT(old).
      if (jmin.gt.2) then
!$OMP MASTER
        dmax=rho(jmin,2,1)
!$OMP END MASTER
...
C***********************************************************************
      SUBROUTINE CENTMASS(X,Y,DISP,ALF)
      IMPLICIT real*8 (a-h,o-z)
      INCLUDE 'hydroparam.h'
      INCLUDE 'globals.h'
      real*8 THETA(lmax+1)
